...
for j = 1:1:6 
    ...
    if j <= 5
        import = readmatrix(strcat('BATCYCLE', string(j),'.CSV'));
    else
        import = readmatrix('BATCYCLE6_CCCV.CSV');
    end
    ...
    if (j==4)
        ...
        set(gca, 'FontName', 'Arial')
        sgtitle(strcat('Battery Characteristic: Cell ', string(j)),'FontName', 'Arial');
        exportgraphics(gcf, strcat('cellcharacteristic',string(j),'.jpg'), 'Resolution',300) 
    end
