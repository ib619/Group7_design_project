// Charging a single cell with a PV panel
  // No Discharging
// Port A: 10R Resistor (not PV panel + 10 R resistor)
// Port B: A single battery
// Aim: Test recalibration algorithm
  // Synchronus
...
float dq1; // used for coulomb counting at the end of each second
float q1; // used for counitng the total capacity
...
void loop() {
  ...
  if (loop_trigger == 1){ // FAST LOOP (200HZ)
      ...
      if (vref == 3600) { // Use Voltage then current PID controller for constant voltage (only in state 6)
        //Serial.println("Using voltage PID controller.");
  ...
  if (int_count == 200) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1:{ // Charge state (Controlled by MPPT Algorithm)
        current_ref = 250;
      ...
      case 4:{ // Discharge rest, no LEDs no current
        ...
        if (rest_timer < 30) { // Rest here for 30s like before
          ...
          mySMPS.send_current_cap(q1, 0, 0);
          q1 = 0;
          next_state = 1;
          digitalWrite(8,true);
          rest_timer = 0;
    ...
    File dataFile = SD.open("Recal.csv", FILE_WRITE); // open our CSV file
  ...
  if (sec_count == 180) {
    if (recalibrating == 1) {
      mySMPS.record_curve(state_num, V_Bat, 0, 0);
