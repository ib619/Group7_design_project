...
MovingAverage<float> intAverage(10);
...
// 7 COMPLETE (blinking yellow LED)
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1: { // Charge state (250mA and a green LED)
          ...
          // Only start logging charge data after complete discharge
          if (discharged == 1) {
            // Write in current voltage value, and measured current
            dataString = String(state_num) + "," + String(V_Bat) + "," + String(current_ref) + "," + String(current_measure);
            Serial.println(dataString);
            File dischargeFile = SD.open(discharge_filename, FILE_WRITE);
            if (dischargeFile) {
              dischargeFile.println(dataString);
            } else {
      ...
      case 3: { //Discharge state (-250mA and no LEDs)
          ...
          dataString = String(state_num) + "," + String(V_Bat) + "," + String(current_ref) + "," + String(current_measure);
          Serial.println(dataString);
          File chargeFile = SD.open(charge_filename, FILE_WRITE);
          if (chargeFile) {
            chargeFile.println(dataString);
          } else {
            Serial.println("File not open");
          }
          chargeFile.close();

          current_cap = current_cap + current_measure / 1000; // FIXME: Adjust for frequency of slow loop if necessary
          break;
        }
      ...
      case 7: { // Complete!
          ...
          dataString = String(current_cap) + "," + String(SoH);
          Serial.println(dataString);
