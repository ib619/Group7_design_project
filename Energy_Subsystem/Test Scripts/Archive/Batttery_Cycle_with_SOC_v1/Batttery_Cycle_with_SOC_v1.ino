...
unsigned int rapid_discharge_timer;
...
void setup() {
  ...
  // Import discharge SoC lookup table;
...
void loop() {
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 6: { // Charging with constant voltage (after state 1, before 2)
        vref = 3600;
        current_ref = 0;
      ...
      case 9: { // Rapid discharge (-1A)
        current_ref = -1000;
         if (V_Bat > 2500 && rapid_discharge_timer < 10) { // while timer is less than 10 seconds
           next_state = 9;
           digitalWrite(8,false);
           rapid_discharge_timer++;
         } else { // If we reach full discharged, move to rest
           rapid_discharge_timer = 0;
           next_state = 8;
           digitalWrite(8,false);
         }
        if(input_switch == 0){ //UNLESS the switch = 0, then go back to start
      ...
      case 10: { // Rapid charge (+500mA, with green LED on)
