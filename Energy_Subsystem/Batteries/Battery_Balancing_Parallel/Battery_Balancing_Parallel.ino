...
// Disconnnect cells which are completelly charged or discharged via a relay
// Discharge doesnt work because it would also divert current away from a less charged cell
...
float V_1, V_2, V_3;
boolean done_1=0, done_2=0, done_3=0; // when it becomes completely empty or compeletely full
...
void loop() {
  ...
  if (int_count == 400 && done_1 == 0) { // Relay switching is 10ms. Double for safety
    digitalWrite(5,true);
  } else if (int_count == 420) { // Read battery 1 voltage
    V_1 = analogRead(A1)*4.096/1.03;
  } else if (int_count == 430 && done_1 == 0) {
    digitalWrite(5,false);
  } else if (int_count == 450 && done_2 == 0) {
    digitalWrite(4,true);
  } else if (int_count == 470) { // Read battery 2 voltage
    V_2 = analogRead(A2)*4.096/1.03;
  } else if (int_count == 480 && done_2 == 0) {
    digitalWrite(4,false);
  } else if (int_count == 500 && done_3 == 0) {
    digitalWrite(9,true);
  } else if (int_count == 520) { // Read battery 3 voltage
    V_3 = analogRead(A3)*4.096/1.03;
  } else if (int_count == 530 && done_3 == 0) {
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1:{ // Charge state (750mA for 3 cells in parallel, and a green LED)
        current_ref = 750;
        if (V_1 > 3600) {  // Cell 1 Lowest
            digitalWrite(5, true);
            done_1 = 1;
          }
          if (V_2 > 3600) { // Cell 2 Lowest
            digitalWrite(4, true);
            done_2 = 1;
          }
          if (V_3>3600) { // Cell 3 Lowest
            digitalWrite(9, true);
            done_3 = 1;
          }
        if (done_1 == 1 && done_2 == 1  && done_3 == 1 ) {
          ...
          done_2 = 0;
          done_3 = 0;
          digitalWrite(8,false); // status LED
          digitalWrite(5, false);
          digitalWrite(4, false);
          digitalWrite(9, false);
        } else {
      ...
      case 3:{ //Discharge state (-750mA and no LEDs)
         current_ref = -750;
         if (V_1 < 2500) {  // Cell 1 Lowest
            digitalWrite(5, true);
            done_1 = 1;
          }
          if (V_2 < 2500) { // Cell 2 Lowest
            digitalWrite(4, true);
            done_2 = 1;
          }
          if (V_3 < 2500) { // Cell 3 Lowest
            digitalWrite(9, true);
            done_3 = 1;
          }
        if (done_1 == 1 && done_2 == 1  && done_3 == 1 ) {
    ...
    dataString = String(state_num) + "," + String(V_1) + "," + String(V_2) + "," + String(V_3) + "," + String(current_ref) + "," + String(current_measure); // + "," + String(done_1)  + "," + String(done_2)  + "," + String(done_3); //build a datastring for the CSV file
