module convolution2(
	...
	output [DATA_WIDTH * 9 - 1:0] o_pixel,
	...
	begin
		if (~rst_n)
			...
			if(state == START_UP) begin
				if (pixelStartUpCounter == width * 3 + 1) 
					begin
						state <= FILTER_ON;
						data_ready <= 1'b0;
						pixelStartUpCounter <= 11'd0;
...
assign o_pixel = { (pixelEnable)? pixel8: 1'd0,
				     	 (pixelEnable)? pixel7: 1'd0,
					    (pixelEnable)? pixel6: 1'd0, 
					    (pixelEnable)? pixel5: 1'd0, 
					    (pixelEnable)? pixel4: 1'd0,  
					    (pixelEnable)? pixel3: 1'd0,
					    (pixelEnable)? pixel2: 1'd0, 
					    (pixelEnable)? pixel1: 1'd0, 
					    (pixelEnable)? pixel0: 1'd0};
endmodule